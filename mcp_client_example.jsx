...
const McpChatbot = () => {
  ...
  const handleMcpMessage = (data) => {
    if (data.type === 'tool/result') {
      ...
      if (result.error) {
        ...
        // 결과를 자연어로 변환
        const naturalResponse = convertResultToNaturalLanguage(result);
        addMessage('AI', naturalResponse, 'info');
      }
  ...
  const convertResultToNaturalLanguage = (result) => {
    // 결과를 자연어로 변환하는 로직
    if (result.playerName) {
      return `${result.playerName} 선수의 ${result.season}시즌 성적입니다:
• 타율: ${result.battingAverage}
• 홈런: ${result.homeRuns}개
• 타점: ${result.rbi}개
• 출루율: ${result.onBasePercentage}
• 장타율: ${result.sluggingPercentage}`;
    } else if (result.teamName) {
      return `${result.teamName}의 현재 순위는 ${result.rank}위입니다:
• 승: ${result.wins}패: ${result.losses}
• 승률: ${result.winPercentage}
• 게임차: ${result.gamesBack}`;
    }
